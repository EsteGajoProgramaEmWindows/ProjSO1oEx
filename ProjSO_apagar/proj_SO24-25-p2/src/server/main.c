  ...
  while(1){
   ...
   if(fd_request == -1) {
    ...
    while(1){
      ...
      if(read_all(fd_response, read_buffer, sizeof(read_buffer), &intr)==-1){
...
static void* manages_register_fifo(void *arguments){
  ...
  while(1){
    ...
    if(opcode == OP_CODE_CONNECT){
      ...
      // Copy the first path (request_fifo_name) correctly
      strncpy(request_fifo_name, buffer + 1, 40);
      request_fifo_name[41] = '\0'; // Ensure null termination
      printf("REQUEST %s\n", request_fifo_name);

      // Copy the second path (response_fifo_name) correctly
      strncpy(response_fifo_name, buffer + 41, 40);
      response_fifo_name[41] = '\0'; // Ensure null termination
      printf("RESPONSE %s\n", response_fifo_name);

      // Copy the third path (notification_fifo_name) correctly
      strncpy(notification_fifo_name, buffer + 81, 40);
      notification_fifo_name[41] = '\0'; // Ensure null termination
      printf("NOTIFICATION %s\n", notification_fifo_name);

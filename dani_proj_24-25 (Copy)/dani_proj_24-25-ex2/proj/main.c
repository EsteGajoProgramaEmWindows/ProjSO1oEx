        ...
        switch (get_next(input_fd)) {
          ...
          case CMD_BACKUP:
             ...
             while(num_backups > args_aux->max_backups){
              ...
              if (result > 0){
                pthread_mutex_lock(&lock_backups);
                num_backups--;
                pthread_mutex_lock(&lock_backups);
              }
            }
            ...
            num_backups++;
            backup_atual++;
            pthread_mutex_unlock(&lock_backups);
            break;
...
int main(int argc, char *argv[]) {
  ...
  // initialize the mutex 
  pthread_mutex_init(&lock_backups, NULL); 
  ...
  // destroy the mutex
  pthread_mutex_destroy(&lock_backups);

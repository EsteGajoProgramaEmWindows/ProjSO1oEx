...
struct HashTable* create_hash_table() {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
      ...
      pthread_mutex_init(&ht->lock_key[i], NULL);
  }
  return ht;
}
...
int write_pair(HashTable *ht, const char *key, const char *value) {
    ...
    while (keyNode != NULL) {
        if (strcmp(keyNode->key, key) == 0) {
            ...
            pthread_mutex_unlock(lock); // free the lock
            return 0;
        }
    ...
    pthread_mutex_unlock(lock); // free the lock after conclusion
    return 0;
}
...
char* read_pair(HashTable *ht, const char *key) {
    ...
    // Locks the mutex
    pthread_mutex_lock(&ht->lock_key[index]); 
    KeyNode *keyNode = ht->table[index];
    char* value;
    ...
    while (keyNode != NULL) {
        if (strcmp(keyNode->key, key) == 0) {
            ...
            // Unlock after conclusion
            pthread_mutex_unlock(&ht->lock_key[index]);
            return value; // Return copy of the value if found
        }
        keyNode = keyNode->next; // Move to the next node
    }
    // Unlock if key not found
    pthread_mutex_unlock(&ht->lock_key[index]); 
...
int delete_pair(HashTable *ht, const char *key) {
    ...
    //Unlock the mutex if key not found
    pthread_mutex_unlock(&ht->lock_key[index]);
    return 1;
}
...
void free_table(HashTable *ht) {
    for (int i = 0; i < TABLE_SIZE; i++) {
        //destroy the lock 
        pthread_mutex_destroy(&ht->lock_key[i]);

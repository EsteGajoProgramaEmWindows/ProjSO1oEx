...
int kvs_subscribe(char *key, int fd_notification){
  if(write_subscription(kvs_table, key, fd_notification) == 0){
    return 0;
  }
  return 1;
...
int kvs_unsubscribe(char *key, int fd_notification){
  if(delete_subscription(kvs_table, key, fd_notification) == 0) {
    return 0;
  }
  return 1; 
}
